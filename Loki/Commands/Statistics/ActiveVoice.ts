...
export default {
    ...
    description: "Sunucudaki anlik ses odasi aktiflik durumunu gosteren premium canli gosterge karti.",
    usage: "aktifses",
    execute: async (client: any, message: Message, args: string[]) => {
        const guildId = message.guild!.id;

        // On yukleme mesaji gonder
        const waitMsg = await message.reply(`${client.emoji("loading") || "⏳"} **Canli ses aktiflik tablosu hazirlaniyor, lutfen bekleyin...**`);

        try {
            const channelsMap = new Map<string, { name: string; userCount: number; members: { name: string; avatarUrl: string; seconds: number }[] }>();
            let totalInVoice = 0;
                ...
                if (ch.isVoiceBased()) {
                    ...
                    if (members.size > 0) {
                        ...
                        const usersList = members.map((m) => {
                            ...
                            let seconds = 0;
                            if (session) {
                                seconds = Math.floor((Date.now() - session.joinedAt) / 1000);
                            }
                            return {
                        ...
                        // Sesteki surelerine gore en yuksek olanı en basa alacak sekilde sirala
            ...
            // CanvasHelper uzerinden canli ses gostergesini ciz
            ...
            await waitMsg.edit("❌ Canli ses aktiflik tablosu hazirlanirken beklenmedik bir hata olustu!").catch(() => {});
        }
    }
};
