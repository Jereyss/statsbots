...
export default {
    name: "server",
    aliases: ["sunucu", "serverstats", "sunucugörüntüle"],
    description: "Sunucu istatistiklerinin genel bir özetini görüntüler.",
    usage: "server",
    execute: async (client: any, message: Message, args: string[]) => {
        const guild = message.guild!;
        const guildId = guild.id;

        const waitMsg = await message.reply("⏳ **Sunucu istatistik paneli hazırlanıyor, lütfen bekleyin...**");
        ...
        try {
            ...
            // 2. Veritabanı İstatistiklerini Çekelim (Performans için .lean() ile)
            ...
            const userVoiceList: { userID: string; value: number }[] = [];
            const userMessageList: { userID: string; value: number }[] = [];
            ...
            for (const stat of allStats) {
                ...
                if (voiceVal > 0) {
                    userVoiceList.push({ userID: stat.userID, value: voiceVal });
                }
                if (msgVal > 0) {
                    userMessageList.push({ userID: stat.userID, value: msgVal });
                }

                // Kanalları topla (Map veya Object durumuna karşı korumalı)
                if (stat.voiceChannels) {
                    ...
                    for (const [chanId, val] of entries) {
                        voiceChannelTotals.set(chanId, (voiceChannelTotals.get(chanId) || 0) + (val as number));
                    }
                }
                if (stat.messageChannels) {
                    ...
                    for (const [chanId, val] of entries) {
                        messageChannelTotals.set(chanId, (messageChannelTotals.get(chanId) || 0) + (val as number));
                    }
                }
            }

            // En aktif kanalı bulalım
            let topVoiceChannelID = "";
            let topVoiceChannelVal = 0;
            for (const [chanId, val] of voiceChannelTotals.entries()) {
                if (val > topVoiceChannelVal) {
                    topVoiceChannelVal = val;
                    topVoiceChannelID = chanId;
                }
            }

            let topMessageChannelID = "";
            let topMessageChannelVal = 0;
            for (const [chanId, val] of messageChannelTotals.entries()) {
                if (val > topMessageChannelVal) {
                    topMessageChannelVal = val;
                    topMessageChannelID = chanId;
                }
            }
            ...
            // En aktif 3 üye (Ses ve Mesaj)
            const topVoiceUsers = userVoiceList
                .sort((a, b) => b.value - a.value)
                .slice(0, 3);

            const topMessageUsers = userMessageList
                .sort((a, b) => b.value - a.value)
                .slice(0, 3);
            ...
            const serverBuffer = await CanvasHelper.drawServerCard({
                ...
                topVoiceUsers: resolvedVoiceUsers,
                topMessageUsers: resolvedMessageUsers
            });
            ...
            console.error("[Server Command] Sunucu kartı oluşturulurken hata:", err);
            await waitMsg.edit("❌ Sunucu istatistik paneli yüklenirken teknik bir hata oluştu!").catch(() => {});
        }
    }
};
